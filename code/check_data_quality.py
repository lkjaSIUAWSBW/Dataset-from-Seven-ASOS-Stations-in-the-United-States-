...
def summarize_dataset(path: Path) -> dict[str, int | str]:
    ...
    # Processed files can contain rows inserted solely to complete the 10-minute time axis.
    # They are not original observations, so exclude them from the requested raw-data QC.
    ...
    # Flag every member of a duplicate group, including the first occurrence.
